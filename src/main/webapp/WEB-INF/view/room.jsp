...
<table>
   ...
   <% for(char i='A';i<=maxRow;i++){ %>                                 <%-- A열부터 시작 --%>
        ...
        <% for(int j=1;j<=maxNum;j++){ %>                               <%-- 해당 열의 1번 좌석부터 시작 --%>
            ...
            <% if(roomMapper.isSeatExist(i,j,r_idx)){         %>      <%-- 좌석이 존재할 때만 버튼을 만든다 --%>
            ...
            <%  String color="";
             ...
             %>
                <button           <%-- IndexController에 seat 매핑된 부분으로 s_idx 전달 --%>
                    ...
                    class="btn btn-xs" onclick="PopupCenterDual('/seat/<%=seatDto.get(index).getS_idx()%>','popup','1000','700');">                         <%-- index에 해당하는 모달창과 연결 --%>
            ...
            <% } %>
            <% if(roomMapper.isHallWay(r_idx,j)){ %>                          <%-- 오른쪽이 복도측인 경우 테이블에 빈 칸을 출력 --%>
