...
<div id="seat-header">
        <h2 style="font-family: 'hanna', serif;"> <%= roomName %> <%=seatRow%>열 <%=seatNum%>번</h4>
        <hr>
      </div>
      <div id="seat-body">
      <% if(session.getAttribute("m_idx")!=null){ %>
        <div class="form-group">
            ...
            <div style="padding-top:5px">
                ...
                <span style="float:right"><button type="button" id ="btnSave" class="btn btn-primary">Save message</button></span>
              </div>
         </div>
      <%  } else {  %>
      <center>로그인 후 리뷰를 작성해주세요!</center>
      <% } %>
      <div id="hr" style="padding-top:10px">
      <hr>
      </div>
            <% if(boardDtoList.size() == 0) { %>
                아직 리뷰가 없습니다.<br>이 자리의 첫 번째 리뷰어가 되어주세요!
            <% } %>
            <% for(int i=0;i<boardDtoList.size();i++){ %>
            <p><%= boardDtoList.get(i).getContent() %>
                <% if(session.getAttribute("m_idx") != null) { %>
                <% if(Integer.parseInt((String) session.getAttribute("m_idx"))==boardDtoList.get(i).getM_idx()) { %>
                    <span id="btnDelete" style="align:right"><button type="button" id="btnDelete<%= boardDtoList.get(i).getB_idx() %>" class="btn btn-default btn-xs" value="<%= boardDtoList.get(i).getB_idx() %>">
                    ×</button></span>
                <%   }} %>
            <%   } %>
        </div>
